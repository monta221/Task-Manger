...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h2>Edit Task</h2>
    <a href="{{ route('projets.taches.index', $tache->projet_id) }}" class="btn btn-secondary">← Back</a>
</div>
...
<form action="{{ route('projets.taches.update', [$tache->projet_id, $tache->tache_id]) }}" method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="mb-3">
        ...
        <select name="projet_id" class="form-control" required>
            @foreach($projets as $projetItem)
                <option value="{{ $projetItem->projet_id }}" {{ $tache->projet_id==$projetItem->projet_id?'selected':'' }}>
